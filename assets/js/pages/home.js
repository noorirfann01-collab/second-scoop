...
(function () {
  ...
  const featured = all.filter(p => p.featured && !p.bundle && !p.topping)
  ...
  const best = all.filter(p => p.badge === "best-seller" && !p.bundle && !p.topping);
  renderGrid("bestseller-grid", best.length ? best : all.filter(p => !p.bundle && !p.topping).slice(0, 3));
  ...
  // --- limited drops ---
  const limited = all.filter(p => !p.bundle && !p.topping && (p.badge === "limited" || p.status === "closing" || p.status === "coming-soon"));
  renderGrid("limited-grid", limited.length ? limited : all.filter(p => !p.bundle && !p.topping).slice(-3));
